...
{   
    ...
    void ReflectionGenerator::prepareStatus(std::string path)
    {
        ...
        // 通过 TemplateManager 加载两个 Mustache 模版
        TemplateManager::getInstance()->loadTemplates(m_root_path, "commonReflectionFile");  // 生成单个类的反射定义
        TemplateManager::getInstance()->loadTemplates(m_root_path, "allReflectionFile");     // 生成全局汇总
        return;
    }
    ...
    int ReflectionGenerator::generate(std::string path, SchemaMoudle schema)
    {
        ...
        std::string    file_path = processFileName(path);  // 拿到要生成的文件名

        // 准备 Mustache 数据容器
        Mustache::data mustache_data;
        Mustache::data include_headfiles(Mustache::data::type::list);  // 保存原始头文件的路径（相对路径） 
        Mustache::data class_defines(Mustache::data::type::list);      // 保存所有类的反射数据
        ...
        for (auto class_temp : schema.classes)
        {
            ...
            for (auto field : class_temp->m_fields)
            {
                ...
                bool is_array = field->m_type.find(vector_prefix) == 0;  // 找到 vector 类型的
                if (is_array)
                {
                    std::string array_useful_name = field->m_type;

                    Utils::formatQualifiedName(array_useful_name);  // 格式化名字
