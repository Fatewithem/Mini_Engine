...
namespace Momo
{
                    ...
                    for (int index = 0; index < array_count; index++)
                    {
                        if (item_ui_creator_iterator == m_editor_ui_creator.end())
                        {
                            ...
                            auto object_instance = Reflection::ReflectionInstance(
                                Momo::Reflection::TypeMeta::newMetaFromName(item_type_meta_item.getTypeName().c_str()),
    ...
    void EditorUI::showEditorDetailWindow(bool* p_open)
    {
        ...
        for (auto component_ptr : selected_object_components)
        {
            ...
            auto object_instance = Reflection::ReflectionInstance(
                Momo::Reflection::TypeMeta::newMetaFromName(component_ptr.getTypeName().c_str()),
    ...
    void DrawVecControl(const std::string& label, Momo::Vector3& values, float resetValue, float columnWidth)
    {
    ...
    void DrawVecControl(const std::string& label, Momo::Quaternion& values, float resetValue, float columnWidth)
    {
        ImGui::PushID(label.c_str());
