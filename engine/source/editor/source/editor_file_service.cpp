    ...
    void EditorFileService::buildEngineFileTree()
    {
        ...
        *root_node     = m_root_node;
        ...
        for (int file_index = 0; file_index < all_file_segments_count; file_index++)
        {
            try
            {
                std::cerr << "Processing file index " << file_index << ": " << file_paths[file_index].generic_string() << std::endl;
            ...
            {
                std::cerr << "Exception caught while processing file index " << file_index << ": " << file_paths[file_index].generic_string() << std::endl;
                std::cerr << "Exception message: " << e.what() << std::endl;
            }
            catch (...)
            {
                std::cerr << "Unknown exception caught while processing file index " << file_index << ": " << file_paths[file_index].generic_string() << std::endl;
            }
        }
    }
