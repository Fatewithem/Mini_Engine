...
namespace Momo
{
    ...
    size_t EditorSceneManager::updateCursorOnAxis(Vector2 cursor_uv, Vector2 game_engine_window_size)
    {
        ...
        else
        {
            ...
            model_matrix.decomposition(model_translation, model_scale, model_rotation);  // 拆分选中轴的矩阵
            ...
            Vector3 world_ray_dir =  // 穿过鼠标像素点的世界空间方向向量 
            ...
            Vector4 local_ray_origin =
                model_matrix.inverse() * Vector4(camera_position, 1.0f);  // 射线的起点
            ...
            Vector3 local_ray_dir        = inversed_rotation * world_ray_dir;  // 射线在 gizmo 局部空间里的方向
