...
namespace Momo
{
    namespace
    {
        ...
        // Serial 往 SkeletonData 里添加一根骨骼
        std::shared_ptr<RawBone> createBone(SkeletonData* skeleton_data, int parent_index)
        {
            RawBone raw_bone;
            raw_bone.index                    = skeleton_data->bones_map.size();    // 顺序编号设定
            raw_bone.parent_index             = parent_index;
            std::shared_ptr<RawBone> bone_ptr = std::make_shared<Momo::RawBone>();
            *bone_ptr                         = raw_bone;
    ...
    std::shared_ptr<Momo::AnimationClip> AnimationLoader::loadAnimationClipData(std::string animation_clip_url)
    {
        AnimationAsset animation_clip;
        g_runtime_global_context.m_asset_manager->loadAsset(animation_clip_url, animation_clip);
        return std::make_shared<Momo::AnimationClip>(animation_clip.clip_data);
    }

    std::shared_ptr<Momo::SkeletonData> AnimationLoader::loadSkeletonData(std::string skeleton_data_url)
    {
        SkeletonData data;
        g_runtime_global_context.m_asset_manager->loadAsset(skeleton_data_url, data);
        return std::make_shared<Momo::SkeletonData>(data);
    }

    std::shared_ptr<Momo::AnimSkelMap> AnimationLoader::loadAnimSkelMap(std::string anim_skel_map_url)
    {
        AnimSkelMap data;
        g_runtime_global_context.m_asset_manager->loadAsset(anim_skel_map_url, data);
        return std::make_shared<Momo::AnimSkelMap>(data);
    }

    std::shared_ptr<Momo::BoneBlendMask> AnimationLoader::loadSkeletonMask(std::string skeleton_mask_file_url)
    {
        BoneBlendMask data;
        g_runtime_global_context.m_asset_manager->loadAsset(skeleton_mask_file_url, data);
        return std::make_shared<Momo::BoneBlendMask>(data);
    }
