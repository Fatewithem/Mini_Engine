...
namespace Momo
{
    ...
    void Skeleton::buildSkeleton(const SkeletonData& skeleton_definition)
    {
        ...
        for (size_t i = 0; i < m_bone_count; i++)
        {
            ...
            m_bones[i].initialize(std::make_shared<RawBone>(bone_definition), parent_bone);  // 初始化骨骼并建立层级关系
        }
    }
    ...
    void Skeleton::applyAnimation(const BlendStateWithClipData& blend_state)
    {
        ...
        {
            const AnimationClip& animation_clip = blend_state.blend_clip[clip_index];           // 存放当前要混合的动画片段
            const float          phase          = blend_state.blend_ratio[clip_index];          // 当前动画播放进度，范围 [0,1]
            const AnimSkelMap&   anim_skel_map  = blend_state.blend_anim_skel_map[clip_index];  // 动画骨骼和运行时骨架的映射表

            float exact_frame        = phase * (animation_clip.total_frame - 1);  // 当前在第几帧
            int   current_frame_low  = floor(exact_frame);                        // 帧下界
            int   current_frame_high = ceil(exact_frame);                         // 帧上界
            float lerp_ratio         = exact_frame - current_frame_low;           // 在这两帧之间的插值因子
                 ...
                 node_index++)
            {
                ...
                // 防止 current_frame_high 超过关键帧数组大小
