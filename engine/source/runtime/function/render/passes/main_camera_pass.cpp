...
namespace Momo
{
    ...
    void MainCameraPass::setupAttachments()
    {
        ...
        m_framebuffer.attachments[_main_camera_pass_gbuffer_a].format          = RHI_FORMAT_R8G8B8A8_UNORM;       // 法线
        m_framebuffer.attachments[_main_camera_pass_gbuffer_b].format          = RHI_FORMAT_R8G8B8A8_UNORM;       // 金属度/粗糙度
        m_framebuffer.attachments[_main_camera_pass_gbuffer_c].format          = RHI_FORMAT_R8G8B8A8_SRGB;        // albedo
        m_framebuffer.attachments[_main_camera_pass_backup_buffer_odd].format  = RHI_FORMAT_R16G16B16A16_SFLOAT;  // 光照结果缓冲
        m_framebuffer.attachments[_main_camera_pass_backup_buffer_even].format = RHI_FORMAT_R16G16B16A16_SFLOAT;  // 后处理缓冲
        ...
        for (int buffer_index = 0; buffer_index < _main_camera_pass_custom_attachment_count; ++buffer_index)
        {
            if (buffer_index == _main_camera_pass_gbuffer_a)
            {
                m_rhi->createImage(m_rhi->getSwapchainInfo().extent.width,
                                   ...
                                   RHI_IMAGE_USAGE_TRANSFER_SRC_BIT,         // 在 CPU 侧可能用到
            ...
            else
            {
                m_rhi->createImage(m_rhi->getSwapchainInfo().extent.width,
                                   ...
                                   RHI_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT,  // 只在 GPU 内部使用
                                   RHI_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
             ...
             ++attachment_index)
        {
            m_rhi->createImage(m_rhi->getSwapchainInfo().extent.width,
                               ...
                               RHI_IMAGE_TILING_OPTIMAL,
                               RHI_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | RHI_IMAGE_USAGE_INPUT_ATTACHMENT_BIT |
                               RHI_IMAGE_USAGE_SAMPLED_BIT,  // 专门用于后处理采样
                               RHI_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
    ...
    void MainCameraPass::setupRenderPass()
    {
        ...
        depth_attachment_description.finalLayout              = RHI_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;  // 专为深度测试/写入
        ...
        // Framebuffer 的输出目标 attchments
        RHIAttachmentReference base_pass_color_attachments_reference[3] = {};
        base_pass_color_attachments_reference[0].attachment = &gbuffer_normal_attachment_description - attachments;  // 索引
        ...
        // EXTERNAL → Deferred Lighting
        RHISubpassDependency& deferred_lighting_pass_depend_on_shadow_map_pass = dependencies[0];
        deferred_lighting_pass_depend_on_shadow_map_pass.srcSubpass           = RHI_SUBPASS_EXTERNAL;  // 依赖来源是外部 RenderPass
        deferred_lighting_pass_depend_on_shadow_map_pass.dstSubpass           = _main_camera_subpass_deferred_lighting;
        deferred_lighting_pass_depend_on_shadow_map_pass.srcStageMask  = RHI_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;  // 等待 color attachment 写完
        deferred_lighting_pass_depend_on_shadow_map_pass.dstStageMask  = RHI_PIPELINE_STAGE_FRAGMENT_SHADER_BIT;  // FS读取
        ...
        // DeferredLighting → ForwardLighting
