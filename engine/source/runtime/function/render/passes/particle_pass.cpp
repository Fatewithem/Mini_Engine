...
namespace Momo
{
    ...
    void ParticlePass::setupAttachments()
    {
        ...
        // momo texture
        {
            std::shared_ptr<TextureData> m_momo_logo_texture_resource = m_render_resource->loadTexture(
                m_particle_manager->getGlobalParticleRes().m_momo_logo_texture_path, true);
            m_rhi->createGlobalImage(m_momo_logo_texture_image,
                                     m_momo_logo_texture_image_view,
                                     m_momo_logo_texture_vma_allocation,
                                     m_momo_logo_texture_resource->m_width,
                                     m_momo_logo_texture_resource->m_height,
                                     m_momo_logo_texture_resource->m_pixels,
                                     m_momo_logo_texture_resource->m_format);
        }
    ...
    void ParticlePass::setupDescriptorSetLayout()
    {
        ...
        // compute descriptor sets
        {
            ...
            {
                RHIDescriptorSetLayoutBinding& momo_texture_layout_binding = particle_layout_bindings[10];
                momo_texture_layout_binding.binding                        = 10;
                momo_texture_layout_binding.descriptorType  = RHI_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
                momo_texture_layout_binding.descriptorCount = 1;
                momo_texture_layout_binding.stageFlags      = RHI_SHADER_STAGE_COMPUTE_BIT;
            }
    ...
    void ParticlePass::updateDescriptorSet()
    {
        for (int eid = 0; eid < m_emitter_count; ++eid)
        {
            // compute part
            {
                ...
                RHIDescriptorImageInfo momo_texture_image_info = {};
                momo_texture_image_info.sampler                = sampler;
                momo_texture_image_info.imageView              = m_momo_logo_texture_image_view;
                momo_texture_image_info.imageLayout            = RHI_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

                {
                    ...
                    descriptorset.pImageInfo             = &momo_texture_image_info;
                    descriptorset.descriptorCount        = 1;
                }
