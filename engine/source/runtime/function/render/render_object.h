...
namespace Momo
{
    ...
    STRUCT(GameObjectMeshDesc, Fields)
    {
        ...
        std::string m_mesh_file;  // 网格资源（模型文件的路径）
    };
    ...
    private:
        GObjectID                       m_go_id {k_invalid_gobject_id};
        std::vector<GameObjectPartDesc> m_object_parts;  // 多个 part 组成
    };
} // namespace Momo

template<>
struct std::hash<Momo::GameObjectPartId>
{
    size_t operator()(const Momo::GameObjectPartId& rhs) const noexcept { return rhs.getHashValue(); }
};
