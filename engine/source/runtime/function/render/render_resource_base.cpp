...
namespace Momo
{
    ...
    {
        // 从 g_runtime_global_context 拿到 asset_manager
        ...
        texture->m_type         = MOMO_IMAGE_TYPE::MOMO_IMAGE_TYPE_2D;
    ...
    StaticMeshData RenderResourceBase::loadStaticMesh(std::string filename, AxisAlignedBox& bounding_box)
    {
        ...
        auto& attrib = reader.GetAttrib();   // 顶点、法线、uv 等属性
        auto& shapes = reader.GetShapes();   // OBJ 中的所有 shape      
