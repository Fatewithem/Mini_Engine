...
namespace Momo
{
    ...
    void MomoEngine::shutdownEngine()
    {
    ...
    void MomoEngine::initialize() {}
    void MomoEngine::clear() {}
    ...
    float MomoEngine::calculateDeltaTime()
    {
        float delta_time;
        {
    ...
    bool MomoEngine::tickOneFrame(float delta_time)
    {
        ...
        g_runtime_global_context.m_window_system->setTitle(
            std::string("Momo - " + std::to_string(getFPS()) + " FPS").c_str());

        const bool should_window_close = g_runtime_global_context.m_window_system->shouldClose();
        return !should_window_close;
    }

    void MomoEngine::logicalTick(float delta_time)
    {
        g_runtime_global_context.m_world_manager->tick(delta_time);
        g_runtime_global_context.m_input_system->tick();
    }

    bool MomoEngine::rendererTick(float delta_time)
    {
        g_runtime_global_context.m_render_system->tick(delta_time);
        return true;
    }

    const float MomoEngine::s_fps_alpha = 1.f / 100;
    void        MomoEngine::calculateFPS(float delta_time)
    {
        m_frame_count++;
