...
namespace Momo
{
    ...
    public:
        ...
        static T*& readPointer(const Json& json_context, T*& instance)
        {
            ...
            if ('*' == type_name[0])  // 如果是普通指针，直接 new T 对象
            {
                instance = new T;
        ...
        {
            T*          instance_ptr = static_cast<T*>(instance.operator->());  // 取出真实指针
            std::string type_name    = instance.getTypeName();                  // 类型名
    ...
    template<>
    char& Serializer::read(const Json& json_context, char& instance);  // char

    template<>
    Json Serializer::write(const int& instance);
    template<>
    int& Serializer::read(const Json& json_context, int& instance);  // int

    template<>
    Json Serializer::write(const unsigned int& instance);
    template<>
    unsigned int& Serializer::read(const Json& json_context, unsigned int& instance);  // uint

    template<>
    Json Serializer::write(const float& instance);
    template<>
    float& Serializer::read(const Json& json_context, float& instance);  // float

    template<>
    Json Serializer::write(const double& instance);
    template<>
    double& Serializer::read(const Json& json_context, double& instance);  // double

    template<>
    Json Serializer::write(const bool& instance);
    template<>
    bool& Serializer::read(const Json& json_context, bool& instance);  // bool 

    template<>
    Json Serializer::write(const std::string& instance);
    template<>
    std::string& Serializer::read(const Json& json_context, std::string& instance);  // string
