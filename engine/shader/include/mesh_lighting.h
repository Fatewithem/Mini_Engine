...
{   
    // 	F0：入射角为 0°（垂直入射）时的反射率（比如绝缘体是 ~0.04，金属来自 basecolor）
    return F0 + (1.0 - F0) * Pow5(1.0 - cosTheta); // 使用5次幂近似
}
                ...
                highp float roughness)
{
    ...
    highp vec3 kD   = (vec3(1.0) - F) * (1.0 - metallic);  // 金属只要镜面，绝缘体才有漫反
